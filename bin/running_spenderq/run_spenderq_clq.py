...
def get_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete the coadd's done marker and existing recon PNG/FITS products before "
            "processing so outputs are recreated from scratch."
        ),
    )
...
def _product_stem(coadd_tag: str, target_id: int, spectrum_index: int) -> str:
    return f"{coadd_tag}_target{int(target_id)}_idx{int(spectrum_index)}"


def _prepare_recon_products(recon_dir: Path, coadd_tag: str) -> Path:
    """Remove stale coadd outputs so the run recreates them from scratch."""
    recon_dir.mkdir(parents=True, exist_ok=True)
    done_marker = recon_dir / f"{coadd_tag}.done"
    done_marker.unlink(missing_ok=True)

    prefix = f"{coadd_tag}_target"
    for child in recon_dir.iterdir():
        if not child.is_file():
            continue
        if not child.name.startswith(prefix):
            continue
        if child.suffix not in {".png", ".fits"}:
            continue
...
def _save_recon_spectrum_fits(
    ...
    observed_hdu = fits.BinTableHDU.from_columns(
        [
    ...
    weight_hdu = fits.BinTableHDU.from_columns(
        [
            fits.Column(name="WAVE_OBS", format="D", array=np.asarray(wave_obs, dtype=np.float64)),
    ...
    recon_hdu = fits.BinTableHDU.from_columns(
        [
    ...
    latent_hdu = fits.ImageHDU(
        data=np.asarray(latent_vector, dtype=np.float64).reshape(1, -1),
        name="LATENT",
    )

    fits.HDUList([primary_hdu, observed_hdu, weight_hdu, recon_hdu, latent_hdu]).writeto(
        out_path,
        overwrite=True,
    )
...
for coadd_path in coadd_files:
    ...
    recon_dir_path = Path(recon_dir)
    if args.refresh_products:
        done_marker = _prepare_recon_products(recon_dir_path, coadd_tag)
    else:
        recon_dir_path.mkdir(parents=True, exist_ok=True)
        done_marker = recon_dir_path / f"{coadd_tag}.done"

    if ENABLE_DONE_MARKERS and done_marker.exists():
    ...
    for i in range(len(target_id_t)):
        ...
        if latent_key in existing_latent_keys and os.path.exists(recon_path) and os.path.exists(recon_fits_path):
