...
for tid, data in recon_store.items():
    ...
    for idx in range(max(len(specs), len(recons))):
        ...
        plt.plot(wv_s, sp, color="slategray", lw=1.0, alpha=0.9, label="Normalized")
        if rc is not None:
            plt.plot(wv_r, rc, color="steelblue", lw=1.15, alpha=0.95, label="SpenderQ recon")
    ...
    if len(specs) > 0 or len(recons) > 0:
        ...
        def _distinct_colors(n, base_cmap="tab20"):
            if n <= 0:
                return []
            if n <= 20:
                cmap_local = plt.get_cmap(base_cmap)
                stride_order = list(range(0, 20, 2)) + list(range(1, 20, 2))
                return [cmap_local(stride_order[i]) for i in range(n)]
            cmap_local = plt.get_cmap("hsv")
            return [cmap_local(i / max(1, n - 1)) for i in range(n)]

        n_obs_colors = max(1, len(specs))
        n_rec_colors = max(1, len(recons))
        obs_colors = _distinct_colors(n_obs_colors, base_cmap="tab20c")
        rec_colors = _distinct_colors(n_rec_colors, base_cmap="tab20")
        ...
        if len(recons) > 0:
            ...
            for j, (wv_r, rc, z_r, obs_r) in enumerate(recons):
                ...
                plt.plot(wv_r, rc, color=colors[j % len(colors)], lw=1.0, label=lbl, alpha=0.3)
        ...
        if len(specs) > 0:
            ...
            for j, (wv_s, sp, z_s, obs_s) in enumerate(specs):
                ...
                plt.plot(wv_s, sp, color=color, lw=1.15, alpha=0.2, linestyle="-", label=f"obs {obs_s} observed")
